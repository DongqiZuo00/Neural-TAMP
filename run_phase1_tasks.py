...
def _print_episode_stats(
    ...
    env_fail_episodes = episode_stats["env_fail_episodes"]
    goal_mismatch_episodes = episode_stats["goal_mismatch_episodes"]
    env_fail_episode_rate = env_fail_episodes / total_episodes if total_episodes else 0.0
    goal_mismatch_rate = goal_mismatch_episodes / total_episodes if total_episodes else 0.0
    print(
        f"[Episode {episode_idx}] env_fail_rate={env_fail_rate:.2%} "
        f"schema_invalid_rate={schema_invalid_rate:.2%} total_steps={total_steps}"
    )
    print(
        f"  env_fail_episode_rate={env_fail_episode_rate:.2%} "
        f"goal_mismatch_rate={goal_mismatch_rate:.2%}"
    )
        ...
        with open(steps_path, "w", encoding="utf-8") as step_handle, open(
            ...
            for scene_index in range(num_scenes):
                ...
                for ep in range(episodes_per_scene):
                    ...
                    if had_env_fail:
                        episode_stats["env_fail_episodes"] += 1
                    if not goal_satisfied:
                        episode_stats["goal_mismatch_episodes"] += 1
