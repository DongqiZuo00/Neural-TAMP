...
from scripts.data_collection import execute_action, export_graph_canonical, get_reachable_positions, graph_diff, sanitize_graph
...
def _sample_action_state_aware(G: nx.DiGraph, rng: random.Random) -> tuple[dict[str, Any], str]:
    ...
    non_openable_nodes = []
    pickupable_nodes = []
    non_pickupable_nodes = []

    for obj_id in objects:
        node_data = G.nodes[obj_id]
        state = _state_dict(node_data)
        geometry = _geometry_dict(node_data)
        open_state = state.get("open_state", "none")
        if geometry.get("openable", False) and open_state == "closed":
            closed_nodes.append(obj_id)
            openable_nodes.append(obj_id)
        elif geometry.get("openable", False) and open_state == "open":
            open_nodes.append(obj_id)
            openable_nodes.append(obj_id)
        else:
            non_openable_nodes.append(obj_id)
        if geometry.get("pickupable", False) and not state.get("held", False):
            pickupable_nodes.append(obj_id)
        else:
            non_pickupable_nodes.append(obj_id)
    ...
    if action == "NavigateTo":
        target = _pick_target(rng, pickupable_nodes or openable_nodes, objects or pos_nodes)
    elif action == "Open":
        if reason == "failure_exploration":
            target = _pick_target(rng, non_openable_nodes, objects)
        else:
        ...
        if reason == "failure_exploration":
            target = _pick_target(rng, non_pickupable_nodes, objects)
        else:
            target = _pick_target(rng, pickupable_nodes, objects)
